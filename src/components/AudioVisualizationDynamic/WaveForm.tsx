...
const WaveForm = ({ analyzerData }: { analyzerData: AnalyserData }) => {
  const canvasRef = useRef<HTMLCanvasElement | null>(null);
  const { dataArray, analyzer, bufferLength } = analyzerData;
  const [width, height] = useSize();

  const draw = (dataArray: Uint8Array<ArrayBuffer>, analyzer: AnalyserNode, bufferLength: number) => {
